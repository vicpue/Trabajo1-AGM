...
function initGame() {
    ...
    var geoempty = new THREE.BoxGeometry(10, 10, 10);
    for (var i = 0; i < ROWS; i++) {
	blocks[i] = [];
	for (var j = 0; j < COLS; j++) {
        ...
        var empty = new THREE.Mesh(geoempty,null);
        if(i%2==0 || j%2==0){
	    blocks[i][j] = { status: 0,
			     object: object 
               }
    ...
    var paddleGeometry = new THREE.BoxGeometry(paddle.width, paddle.height, 20);
